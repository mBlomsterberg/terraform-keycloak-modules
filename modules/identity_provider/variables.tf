...
variable "alias" {
  description = "Unique alias identifying this identity provider within the realm. Used as the URL fragment and as the value referenced by mappers."
  type        = string
}
...
variable "trust_email" {
  description = "When true, email addresses returned by the provider are trusted without requiring verification."
  type        = bool
  default     = false
}
...
variable "provider_id" {
  description = "Internal Keycloak provider type identifier. Defaults to \"oidc\"."
  type        = string
  default     = "oidc"
}

variable "authorization_url" {
  description = "URL of the OIDC authorisation endpoint."
  type        = string
}

variable "client_id" {
  description = "Client ID registered with the upstream OIDC provider."
  type        = string
}

variable "client_secret" {
  description = "Client secret for the upstream OIDC provider. Mutually exclusive with client_secret_wo / client_secret_wo_version."
  type        = string
  sensitive   = true
  default     = null
}
...
variable "token_url" {
  description = "URL of the OIDC token endpoint."
  type        = string
}

variable "user_info_url" {
  description = "URL of the OIDC UserInfo endpoint."
  type        = string
  default     = null
}

variable "jwks_url" {
  description = "URL from which Keycloak retrieves the provider's JSON Web Key Set for signature validation."
  type        = string
  default     = null
}

variable "issuer" {
  description = "Expected issuer claim in tokens from this provider. Used for validation when validate_signature is enabled."
  type        = string
  default     = null
}

variable "logout_url" {
  description = "URL of the provider's end-session (logout) endpoint."
  type        = string
  default     = null
}
...
variable "default_scopes" {
  description = "Space-separated list of OIDC scopes to request by default (e.g. \"openid profile email\")."
  type        = string
  default     = null
}

variable "validate_signature" {
  description = "When true, Keycloak validates the signature of tokens from this provider using jwks_url or the issuer's discovery document."
  type        = bool
  default     = false
}

variable "disable_type_claim_check" {
  description = "When true, Keycloak skips the check that the token type claim equals \"Bearer\"."
  type        = bool
  default     = false
}

variable "backchannel_supported" {
  description = "When true, the provider supports backchannel logout."
  type        = bool
  default     = true
}

variable "login_hint" {
  description = "Login hint to pre-populate the username field on the provider's login page."
  type        = string
  default     = null
}

variable "ui_locales" {
  description = "When true, Keycloak forwards the user's UI locale to the provider as the ui_locales parameter."
  type        = bool
  default     = false
}

variable "accepts_prompt_none_forward_from_client" {
  description = "When true, a prompt=none request from a client is forwarded to the provider rather than handled locally."
  type        = bool
  default     = false
}
