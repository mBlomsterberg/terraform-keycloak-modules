locals {
  idp_alias = try(keycloak_oidc_google_identity_provider.this[0].alias, null)
}
...
resource "keycloak_oidc_google_identity_provider" "this" {
  ...
  use_user_ip_param                       = var.use_user_ip_param
  request_refresh_token                   = var.request_refresh_token
}
