...
variable "alias" {
  description = "Unique alias identifying this identity provider within the realm. Defaults to \"microsoft\" if not set."
  type        = string
  default     = "microsoft"
}
...
variable "trust_email" {
  description = "When true, email addresses returned by Microsoft are trusted without requiring verification."
  type        = bool
  default     = false
}
...
variable "client_secret" {
  description = "Azure AD client secret. Mutually exclusive with client_secret_wo / client_secret_wo_version."
  type        = string
  sensitive   = true
  default     = null
}
...
variable "default_scopes" {
  description = "Space-separated list of OIDC scopes to request. Defaults to \"openid profile email\"."
  type        = string
  default     = "openid profile email"
}

variable "login_hint" {
  description = "Login hint to pre-populate the username field on the Microsoft login page."
  type        = string
  default     = null
}

variable "accepts_prompt_none_forward_from_client" {
  description = "When true, a prompt=none request from a client is forwarded to Microsoft rather than handled locally."
  type        = bool
  default     = false
}
...
variable "tenant_id" {
  description = "Azure AD tenant ID. Set to restrict sign-in to a specific tenant. Leave null for multi-tenant (common) endpoint."
  type        = string
  default     = null
}

variable "prompt" {
  description = "Prompt parameter sent to Azure AD: login, none, consent, or select_account."
  type        = string
  default     = null

  validation {
    condition     = var.prompt == null || contains(["login", "none", "consent", "select_account"], var.prompt)
    error_message = "prompt must be one of: login, none, consent, select_account."
  }
}
